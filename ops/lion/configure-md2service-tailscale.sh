...
origin="${MD2SERVICE_ORIGIN:-http://127.0.0.1:1881}"
...
if ! command -v tailscale >/dev/null 2>&1; then
  echo "tailscale is required on the Lion server before configuring Serve." >&2
  exit 1
fi

if command -v curl >/dev/null 2>&1; then
  if ! curl -fsS --max-time 5 "${origin}/" >/dev/null; then
    echo "Warning: Systems did not respond at ${origin}/ from this host." >&2
    echo "Confirm the local Systems service is running before relying on remote access." >&2
  fi
fi
...
Cloudflare Tunnel is the preferred public route for md2service.facilities-engineering.com.
